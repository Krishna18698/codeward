      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-8 shadow-2xl animate-scale-in">
        ...
        {step === 0 && (
          <div className="animate-fade-in">
            <h2 className="text-xl font-semibold tracking-heading text-primary mb-1">Welcome to Codeward 👋</h2>
            <p className="text-secondary text-sm mb-6">Quick setup — takes 20 seconds. Helps the AI mentor tailor your prep.</p>
            <p className="text-xs font-semibold text-muted uppercase tracking-wider mb-3">Your experience level</p>
            <div className="space-y-2">
              {EXP_LEVELS.map((l) => {
                ...
                return (
                  ...
                  >
                    {selected && (
                      <CheckCircle2 size={14} className="absolute right-3 top-3 text-accent" />
                    )}
        ...
        {step === 1 && (
          <div className="animate-fade-in">
            <h2 className="text-xl font-semibold tracking-heading text-primary mb-1">Target company</h2>
            <p className="text-secondary text-sm mb-6">The AI mentor will bias your study plan toward this company&apos;s interview style.</p>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {COMPANIES.map((c) => {
                ...
                return (
                  ...
                  >
                    {selected && (
                      <CheckCircle2 size={10} className="absolute right-2 top-2 text-accent" />
                    )}
                    {c}
                  </button>
                );
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setStep(0)}
                className="flex-1 rounded-xl border border-border text-secondary text-sm py-2.5 hover:border-border transition-colors"
              >
                ← Back
              </button>
              <button
