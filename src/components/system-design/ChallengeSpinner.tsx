...
const SCALES = [
  ...
  { label: "1M DAU",   desc: "Mid-size product scale",    color: "text-sky-400",     bg: "bg-sky-500/10 border-sky-500/20" },
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">Scale · spike · constraints — all random</p>
          </div>
          <button
            onClick={spin}
            disabled={spinning}
        ...
        {challenge && revealed && (
          <div className="border-t border-slate-800/60 px-4 py-3 space-y-2.5 animate-fade-up">
            {/* Problem */}
            ...
            <div className={cn("inline-flex items-center gap-1.5 rounded-lg border px-2 py-1 text-[11px] font-medium", challenge.scale.bg)}>
              ...
              <span className="text-slate-500">— {challenge.scale.desc}</span>
            </div>
            ...
            <div className="rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2.5">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                ...
                Account for: <span className="text-slate-300">{challenge.spike.toLowerCase()}</span>.{" "}
                Key requirement: <span className="text-slate-300">{challenge.constraint.toLowerCase()}</span>.
              </p>
            </div>
          </div>
        )}
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/50 overflow-hidden">
      ...
      {spinning && (
        <div className="px-5 py-12 text-center">
          ...
          <p className="text-sm text-slate-500">Generating your challenge…</p>
        </div>
      )}
