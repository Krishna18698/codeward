  ...
  return (
    <div className="flex h-[calc(100vh-0px)] overflow-hidden -m-8">
      ...
      <div className="w-[420px] shrink-0 flex flex-col border-r border-neutral-800 bg-canvas overflow-hidden">
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-neutral-800 text-xs text-neutral-500 shrink-0">
          <Link href="/dashboard/system-design" className="hover:text-neutral-300 transition">
            System Design
          </Link>
          <span>/</span>
          <span className="text-neutral-400 truncate">{question.title}</span>
        </div>

        {/* Question header */}
        <div className="px-4 pt-4 pb-3 border-b border-neutral-800/60 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {tab === "prompt" && (
            <div className="px-4 py-4 space-y-4">
              <p className="text-sm text-neutral-300 leading-relaxed">{question.description}</p>
              ...
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3">
                <p className="text-xs text-emerald-400 font-medium mb-1">✦ Ask the mentor</p>
          ...
          {tab === "framework" && (
            <div className="px-4 py-4 space-y-3">
              ...
              {FRAMEWORK.map((f) => (
                <div key={f.step} className="flex gap-3 rounded-xl border border-neutral-800 bg-neutral-900/50 p-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">{f.hint}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className={`flex-1 flex flex-col overflow-hidden ${showMentor ? "" : "hidden xl:flex"}`}>
        {/* Mentor header bar */}
        <div className="flex items-center justify-between px-4 py-2 border-b border-neutral-800 bg-canvas shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-emerald-400 text-sm">✦</span>
            <span className="text-sm font-medium text-white">AI Mentor</span>
            <span className="text-xs text-neutral-500">— ask me to walk through this design</span>
          </div>
          <button
            onClick={() => setShowMentor((v) => !v)}
            className="text-neutral-500 hover:text-neutral-400 text-xs xl:hidden"
          >
            ✕
          </button>
        </div>
