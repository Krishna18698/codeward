  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-1.5 border-b border-border shrink-0">
        <span className="text-[11px] text-muted">Design notes, diagrams in text, trade-offs</span>
        <span className={`text-[11px] transition ${
          saveState === "saving" ? "text-accent" :
          saveState === "saved"  ? "text-accent" : "text-muted"
        }`}>
      ...
      <textarea
        ...
        className="flex-1 w-full resize-none bg-transparent px-4 py-3 text-sm text-secondary placeholder-neutral-800 outline-none font-mono leading-relaxed"
        spellCheck={false}
      />
    </div>
  );
