...
function Ring({ pct, size = 56, stroke = 4, color = "#0ea5e9" }: { pct: number; size?: number; stroke?: number; color?: string }) {
...
export default async function DashboardPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-up">
        ...
        {/* ── Hero banner ── */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-linear-to-br from-slate-900 via-slate-900/95 to-sky-950/40 p-6">
          {/* Background accent */}
          <div className="pointer-events-none absolute right-0 top-0 h-full w-1/2"
               style={{ background: "radial-gradient(ellipse 80% 80% at 100% 50%, rgba(14,165,233,0.08) 0%, transparent 70%)" }} />
            ...
            <div className="flex items-center gap-4">
              ...
              {userImage ? (
                <Image
                  ...
                  className="rounded-full border-2 border-sky-500/30 shrink-0"
                />
              ) : (
                <div className="w-13 h-13 rounded-full border-2 border-sky-500/30 bg-linear-to-br from-sky-500/20 to-indigo-500/20 flex items-center justify-center text-white font-bold text-lg shrink-0">
                  {firstName[0]}
                </div>
              )}
              ...
              <div>
                ...
                {user.targetCompany && (
                  <p className="text-xs text-slate-500 mt-0.5">
                    Targeting <span className="text-sky-400 font-medium">{user.targetCompany}</span>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-xs text-slate-500">{doneCount} / {totalTracked} problems</span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div
              key={stat.label}
              className={`relative overflow-hidden rounded-2xl border ${stat.border} bg-slate-900/60 p-5 animate-fade-up`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link
            href="/dashboard/dsa"
            className="group flex items-center gap-4 rounded-2xl border border-slate-800 bg-slate-900/50 p-4 hover:border-sky-500/30 hover:bg-slate-900/80 transition-all duration-200"
          >
            <div className="rounded-xl bg-sky-500/10 p-3 shrink-0 group-hover:bg-sky-500/20 transition-colors">
              <Code2 size={20} className="text-sky-400" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-white">DSA Practice</p>
              <p className="text-xs text-slate-500 truncate">{doneCount} solved</p>
            </div>
            <ArrowRight size={14} className="text-slate-700 group-hover:text-sky-400 group-hover:translate-x-0.5 transition-all ml-auto shrink-0" />
          </Link>

          <Link
            href="/dashboard/system-design"
            className="group flex items-center gap-4 rounded-2xl border border-slate-800 bg-slate-900/50 p-4 hover:border-indigo-500/30 hover:bg-slate-900/80 transition-all duration-200"
          >
            <div className="rounded-xl bg-indigo-500/10 p-3 shrink-0 group-hover:bg-indigo-500/20 transition-colors">
              <Network size={20} className="text-indigo-400" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-white">System Design</p>
              <p className="text-xs text-slate-500 truncate">{sdTotal} questions · 3 levels</p>
            </div>
            <ArrowRight size={14} className="text-slate-700 group-hover:text-indigo-400 group-hover:translate-x-0.5 transition-all ml-auto shrink-0" />
          </Link>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Your Sheets</h2>
            <Link href="/dashboard/dsa" className="text-xs text-sky-400 hover:text-sky-300 transition-colors">View all →</Link>
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            {/* AI build card */}
            <div className="rounded-2xl border border-dashed border-sky-500/25 bg-sky-500/5 p-5 flex flex-col gap-2.5 hover:border-sky-500/50 hover:bg-sky-500/8 transition-all duration-200 group">
              ...
              <Link href="/dashboard/mentor" className="inline-flex items-center gap-1 self-start text-xs font-medium text-sky-400 hover:text-sky-300 transition-colors mt-0.5 group-hover:gap-2">
            ...
            {sheets.slice(0, 3).map((sheet, i) => {
              ...
              return (
                <Link
                  ...
                  className="group rounded-2xl border border-slate-800 bg-slate-900/50 p-5 hover:border-slate-600 hover:bg-slate-900/80 transition-all duration-200 animate-fade-up"
                  style={{ animationDelay: `${i * 60}ms` }}
                >
        ...
        {topPatterns.length > 0 ? (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pattern Breakdown</h2>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/50 divide-y divide-slate-800/60">
              {topPatterns.map(([pattern, { done, total }]) => {
                const pct = total > 0 ? Math.round((done / total) * 100) : 0;
                return (
                  <div key={pattern} className="flex items-center gap-4 px-5 py-3 group hover:bg-slate-800/20 transition-colors">
                    <span className="w-44 text-xs text-slate-400 capitalize group-hover:text-slate-300 transition-colors">
