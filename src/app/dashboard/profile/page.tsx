  ...
  return (
    <div className="max-w-2xl space-y-6 animate-fade-up">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              {user.experienceLevel && (
                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-400 border border-emerald-500/30 bg-emerald-500/10 rounded-full px-2.5 py-0.5">
                  <User size={10} />
                  {user.experienceLevel.charAt(0) + user.experienceLevel.slice(1).toLowerCase()}
                </span>
              )}
              {user.targetCompany && (
                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-rose-400 border border-rose-500/30 bg-rose-500/10 rounded-full px-2.5 py-0.5">
                  <Target size={10} />
                  {user.targetCompany}
                </span>
              )}
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50 p-6">
        <h2 className="text-sm font-semibold text-neutral-300 mb-5">Edit profile</h2>
